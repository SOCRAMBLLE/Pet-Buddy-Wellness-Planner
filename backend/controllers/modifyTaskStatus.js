...
exports.modifyTaskStatus = async (req, res) => {
  const { petID, taskID, newStatus } = req.body;
  ...
  try {
    ...
    // update description
    data.Item.ToDoList[taskIndex].Completed = newStatus;
    ...
    res.status(500).json({ error: 'Error while updating task status.' });
  }
};
